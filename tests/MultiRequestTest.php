	...
	function test() {
		...
		$rtn   = $mc->addOptions(
			array(
				array(
					...
					'callback' => function (Response $response) {
						...
						$this->assertFalse($response->hasErrors(), $response->request->uri.$response->error);
						$this->assertEquals(TEST_SERVER.'/dynamic/blocking.php?&1', $response->request->uri);
				...
				array(
					...
					'callback' => function (Response $response) {
						$this->assertFalse($response->hasErrors(), $response->request->uri.$response->error);
						$this->assertEquals(TEST_SERVER.'/dynamic/blocking.php?&b', $response->request->uri);
				...
				array(
					...
					'timeout'  => 3,
					'callback' => function (Response $response) {
						$this->assertContains('http://www.qq.com', $response->request->uri);
						$this->assertTrue($response->request->hasEndCallback());
					},
				),
				array(
			...
			->add(\MultiHttp\Http::GET, 'http://www.163.com', array(), array(
				'timeout'  => 3,
				'callback' => function (Response $response) {
					$this->assertContains('http://www.163.com', $response->request->uri);
				},
			))
			...
			->import(Request::create()->trace('http://sohu.cn', array(
					'timeout'  => 3,
					'callback' => function (Response $response) {
						$this->assertContains('http://sohu.cn', $response->request->uri);
					}))->applyOptions())
		      ->import(Request::create()->options('http://toutiao.com', array(
					'timeout'  => 3,
					'callback' => function (Response $response) {
						$this->assertContains('http://toutiao.com', $response->request->uri);
					}))->applyOptions())
		      ->execute();
		echo "exec done\n\t";
